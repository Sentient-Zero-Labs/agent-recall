...
class TestDecayScoring:
    async def test_decay_score_null_before_first_run(self, client, namespace):
        """A freshly stored memory has no decay_score yet — DecayWorker has not run."""
        m = await client.store(namespace, "User prefers Python for backend work", "tech")
    ...
    async def test_decay_score_set_after_run(self, client, namespace, decay_worker):
        """run_once() writes a decay_score to all active memories."""
        await client.store(namespace, "User prefers dark mode in IDE", "prefs")
        count = await decay_worker.run_once()
        assert count >= 1
        async with aiosqlite.connect(get_db_path()) as db:
            rows = await db.execute_fetchall(
                "SELECT decay_score FROM memories WHERE namespace = ? AND valid_until IS NULL",
                (namespace,),
            )
        for (score,) in rows:
    ...
    async def test_old_memory_has_lower_decay_than_new(self, client, namespace, decay_worker):
        """A 90-day-old memory should have a lower decay_score than a fresh one."""
        old = await client.store(namespace, "User liked Java for enterprise work", "tech")
        new = await client.store(namespace, "User prefers TypeScript for frontend", "tech")
    ...
    async def test_frequently_accessed_memory_decays_slower(self, client, namespace, decay_worker):
        """A memory accessed 10+ times should retain higher decay_score than one never accessed."""
        low_ac = await client.store(namespace, "User prefers light theme in VSCode", "prefs")
        high_ac = await client.store(namespace, "User prefers dark theme in VSCode", "prefs")
    ...
    async def test_decay_does_not_modify_importance(self, client, namespace, decay_worker):
        """DecayWorker updates decay_score only — importance is never modified."""
        m = await client.store(namespace, "User is a senior software engineer at Acme Corp", "work")
    ...
    async def test_decay_only_affects_active_memories(self, client, namespace, decay_worker):
        """Superseded memories (valid_until set) must not get a decay_score."""
        active = await client.store(namespace, "User uses FastAPI for APIs", "tech")
        superseded = await client.store(namespace, "User used Flask for APIs", "tech")
    ...
    async def test_run_once_returns_count(self, client, namespace, decay_worker):
        """run_once() must return the exact count of memories it updated."""
        await client.store(namespace, "memory one", "general")
        await client.store(namespace, "memory two", "general")
        await client.store(namespace, "memory three", "general")
    ...
    async def test_decay_score_affects_search_ranking(self, client, namespace, decay_worker):
        ...
        fresh = await client.store(namespace, "Python is the best language for data science", "tech")
        stale = await client.store(namespace, "Python is the best language for data science work", "tech")
        ...
        # recency_weight=0, mmr_lambda=1.0 → scoring driven purely by relevance + importance * decay_score
        # mmr_lambda=1.0 disables diversity penalty so MMR doesn't override decay-aware ordering
        results = await _hybrid_search(namespace, "Python data science language", limit=10, recency_weight=0.0, mmr_lambda=1.0)
