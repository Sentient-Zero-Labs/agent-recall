...
class TestMMR:
    async def test_mmr_deduplicates_near_duplicates(self, client, namespace):
        ...
        dup1 = await client.store(namespace, "I prefer Python for all backend API development work", "tech")
        dup2 = await client.store(namespace, "I prefer Python for backend API development projects", "tech")
        distinct = await client.store(namespace, "I use dark mode in VS Code editor", "tools")

        results = await _hybrid_search(
            namespace, "Python backend API", limit=3, recency_weight=0.0, mmr_lambda=0.3
        )
    ...
    async def test_mmr_lambda_1_preserves_relevance_order(self, client, namespace):
        """λ=1.0 degrades to pure relevance — output order equals non-MMR hybrid ranking."""
        m1 = await client.store(namespace, "Python FastAPI backend framework development", "tech")
        m2 = await client.store(namespace, "Python Flask web development API", "tech")
        m3 = await client.store(namespace, "JavaScript React frontend UI components", "tech")

        # Get baseline (no MMR, just hybrid)
        baseline = await _hybrid_search(
            namespace, "Python backend", limit=3, recency_weight=0.0, mmr_lambda=1.0
        )
    ...
    async def test_mmr_lambda_0_reduces_redundancy(self, client, namespace):
        """λ=0.0 maximises diversity — of 3 near-identical memories, at most 1 should be selected."""
        # Three near-identical memories
        for i in range(3):
            await client.store(
                namespace, f"I always use Python for backend development work variation {i}", "tech"
            )
        # One clearly distinct memory
        distinct = await client.store(namespace, "I enjoy hiking in national parks on weekends", "personal")

        results = await _hybrid_search(
            namespace, "Python backend development", limit=2, recency_weight=0.0, mmr_lambda=0.0
        )
    ...
    async def test_score_threshold_excludes_low_scoring_candidates(self, client, namespace):
        """A very high score_threshold drops everything below it."""
        await client.store(namespace, "I prefer Python for backend development", "tech")

        # threshold=999.0 is impossibly high — nothing can score that high
        results = await _hybrid_search(
            namespace, "Python backend", limit=10, recency_weight=0.3, score_threshold=999.0
        )
        assert results == [], "Impossibly high threshold should return empty results"

    async def test_mmr_fallback_when_embeddings_unavailable(self, client, namespace, monkeypatch):
        """When embed() returns None, MMR falls back to top-limit candidates by hybrid score."""
        await client.store(namespace, "I prefer Python for backend API work", "tech")
        await client.store(namespace, "Python development is my main stack", "tech")
        ...
        results = await _hybrid_search(
            namespace, "Python backend", limit=5, recency_weight=0.0, mmr_lambda=0.3
        )
